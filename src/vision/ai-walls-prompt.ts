...
4. 先划掉标注：所有带数字的尺寸线（1950、3140、3320、开间、进深、18670）、两端短刻度、尺寸界线、图幅四周那一整圈尺寸链。这些绝对不要进 outerLoop 或 innerWalls。黄线/黄字只做 OCR
5. 粗承重墙：比尺寸线明显更粗的实心段、双线，以及斜线填充/灰色实心厚块/小柱。外墙之后先把这些全部描完（画中心线）。带拐弯/错位的必须在每个拐角落点，禁止拉成一条直线。厨房/餐厅中间那些细竖线是标注，不是墙
...
export const AI_WALLS_USER_PROMPT = (
  ...
  [
    ...
    '尺寸数字旁边的细线不是墙。1950、2000、3140、3320、18670、开间、进深、两端短竖线/箭头，以及图幅最外一圈尺寸链，全部丢掉，不要当 outerLoop。黄线/黄字只读数。厨房和餐厅里最容易把这些竖标注画成墙，禁止。',
    '外墙之后最重要的是承重墙：比标注细线明显更粗的实心段或双线（CAD 黑底则是粗白/粗青双线），以及斜线填充、灰色实心厚块和小柱。走廊、分户、书房与客厅之间的 L 形厚墙、厨卫周围这些粗线必须整条描进 innerWalls（填充块画中心线，不要只描外框）。',
